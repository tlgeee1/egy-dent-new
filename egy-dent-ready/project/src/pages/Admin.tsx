  ...
  return (
    <div className="grid min-h-screen place-items-center bg-ink-950 p-4">
      ...
      <motion.form
        ...
        className="relative w-full max-w-sm rounded-[2rem] border border-white/10 bg-ink-900 p-8 text-center shadow-2xl"
      >
        <span className="mx-auto grid size-16 place-items-center rounded-2xl bg-gradient-to-br from-volt-400 to-volt-700 shadow-[0_12px_32px_rgba(34,211,238,0.35)]">
          <ToothMark className="size-9 text-ink-950" />
        </span>
        ...
        <div className="relative mt-6">
          ...
          <input
            ...
            className={cn(
              ...
              err ? "border-red-400/60" : "border-white/10 focus:border-volt-500/60",
            )}
          />
        </div>
        {err && <p className="mt-2 text-xs text-red-400">كلمة المرور غير صحيحة</p>}
        <button className="mt-4 w-full rounded-2xl bg-gradient-to-l from-volt-400 to-volt-600 py-3.5 font-display font-black text-ink-950 transition-transform hover:scale-[1.02] active:scale-95">
          دخول
        </button>
        <p className="mt-4 rounded-xl bg-white/[0.04] py-2 text-[11px] text-frost-500">
...
function ProductForm({
  ...
  const field = "w-full rounded-2xl border border-white/10 bg-ink-950/60 px-4 py-3 text-sm outline-none transition-colors placeholder:text-frost-500/70 focus:border-volt-500/60";
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center overflow-y-auto p-4">
      ...
      <motion.div
        ...
        className="relative my-6 w-full max-w-2xl rounded-[2rem] border border-white/10 bg-ink-900 p-6 shadow-2xl md:p-8"
      >
        <div className="flex items-center justify-between">
          <h3 className="font-display text-xl font-black">{initial.name ? "تعديل منتج" : "إضافة منتج جديد"}</h3>
          <button onClick={onClose} className="grid size-9 place-items-center rounded-xl border border-white/10 text-frost-400 hover:bg-white/5">
            <X className="size-4" />
          </button>
        </div>
              ...
              {IMAGE_CHOICES.map((img) => (
                <button
                  ...
                  className={cn(
                    ...
                    d.img === img ? "border-volt-400 ring-2 ring-volt-400/30" : "border-white/10 opacity-60 hover:opacity-100",
                  )}
                >
        ...
        <div className="mt-6 flex gap-3">
          <button onClick={save} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-volt-400 to-volt-600 py-3.5 font-display font-black text-ink-950 transition-transform hover:scale-[1.02]">
            <Save className="size-4" />
            حفظ المنتج
          </button>
          <button onClick={onClose} className="rounded-2xl border border-white/10 px-6 py-3.5 text-sm font-bold text-frost-300 hover:bg-white/5">
            إلغاء
          </button>
        </div>
      </motion.div>
...
export default function Admin() {
  ...
  return (
    <div className="noise min-h-screen bg-ink-950 font-sans text-white">
      {/* header */}
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-4">
          <div className="flex items-center gap-3">
            <span className="grid size-10 place-items-center rounded-xl bg-gradient-to-br from-volt-400 to-volt-700">
              <ToothMark className="size-5 text-ink-950" />
            </span>
            <div>
          ...
          <div className="flex items-center gap-2.5">
            <a href="#" className="flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-xs font-bold text-frost-300 transition-colors hover:border-volt-500/40 hover:text-volt-300">
          ...
          {tabs.map((t) => (
            <button
              ...
              className={cn(
                ...
                tab === t.id ? "bg-volt-500 text-ink-950 shadow-[0_8px_22px_rgba(34,211,238,0.35)]" : "text-frost-400 hover:bg-white/5 hover:text-white",
              )}
            >
      ...
      <main className="mx-auto max-w-6xl px-5 py-8">
        ...
        {tab === "overview" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              {stats.map((s) => (
                <div key={s.label} className="rounded-[1.5rem] border border-white/[0.07] bg-ink-900 p-5">
            ...
            {store.orders.length === 0 ? (
              <p className="mt-4 rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-frost-500">
                لسه مفيش طلبات — أول ما عميل يطلب من المتجر هيظهر هنا فوراً
              </p>
            ) : (
              <div className="mt-4 space-y-3">
                {store.orders.slice(0, 5).map((o) => (
                  <div key={o.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/[0.07] bg-ink-900 p-4">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-display text-base font-black text-white">{fmt(o.total)} ج</span>
                ...
                {store.orders.length > 5 && (
                  <button onClick={() => setTab("orders")} className="w-full rounded-2xl border border-white/10 py-3 text-sm font-bold text-frost-300 hover:bg-white/5">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-2.5">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-gradient-to-l from-volt-400 to-volt-600 px-5 py-2.5 text-xs font-black text-ink-950 transition-transform hover:scale-105"
                >
            ...
            <div className="mt-5 overflow-x-auto rounded-2xl border border-white/[0.07]">
              <table className="w-full min-w-[640px] text-right text-sm">
                <thead>
                  <tr className="border-b border-white/[0.07] bg-ink-900 text-xs text-frost-500">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button
                            ...
                            className="grid size-9 place-items-center rounded-xl border border-white/10 text-frost-300 transition-colors hover:border-volt-500/50 hover:text-volt-300"
                            aria-label="تعديل"
                          >
                          ...
                          <button
                            ...
                            className="grid size-9 place-items-center rounded-xl border border-white/10 text-frost-300 transition-colors hover:border-red-400/50 hover:text-red-400"
                            aria-label="حذف"
                          >
        ...
        {tab === "orders" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            {store.orders.length === 0 ? (
              <p className="rounded-2xl border border-dashed border-white/10 p-12 text-center text-sm text-frost-500">
                مفيش طلبات لسه — جرّب اطلب حاجة من المتجر وهتلاقيها هنا
              </p>
            ) : (
              <div className="space-y-4">
                {store.orders.map((o: Order) => (
                  <div key={o.id} className="overflow-hidden rounded-[1.5rem] border border-white/[0.07] bg-ink-900">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        >
                          {(["جديد", "مؤكد", "تم الشحن", "مكتمل"] as OrderStatus[]).map((s) => (
                            <option key={s} value={s} className="bg-ink-900 text-white">{s}</option>
                          ))}
                        </select>
                        <button
                          onClick={() => {
                          ...
                          className="grid size-9 place-items-center rounded-xl border border-white/10 text-frost-400 hover:border-red-400/50 hover:text-red-400"
                    ...
                    <div className="grid gap-4 p-4 md:grid-cols-2">
                      <div className="space-y-2 text-sm">
                        <p className="font-extrabold text-white">{o.customer.name}</p>
                        ...
                        {o.customer.notes && <p className="rounded-xl bg-white/[0.04] p-2.5 text-xs text-frost-500">ملاحظات: {o.customer.notes}</p>}
                        <p className="inline-flex rounded-full border border-white/10 px-3 py-1 text-[11px] font-bold text-frost-300">{o.payment}</p>
                      </div>
                      <div className="space-y-1.5 text-xs">
                        ...
                        <div className="flex justify-between border-t border-white/[0.07] pt-2 font-display text-base font-black text-white">
                          <span>الإجمالي</span>
        ...
        {tab === "settings" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-xl">
            <div className="rounded-[1.5rem] border border-white/[0.07] bg-ink-900 p-6">
              ...
              <div className="mt-5 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    dir="ltr"
                    className="w-full rounded-2xl border border-white/10 bg-ink-950/60 px-4 py-3 text-left text-sm outline-none focus:border-volt-500/60"
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-volt-400 to-volt-600 py-3.5 font-display font-black text-ink-950 transition-transform hover:scale-[1.02]"
                >
                  {saved ? (
                    <>
