...
export default function CheckoutModal() {
  ...
  const inputCls = (hasErr: boolean) =>
    cn(
      ...
      hasErr ? "border-red-400/60" : "border-white/10 focus:border-volt-500/60",
    );
      ...
      {checkoutOpen && (
        <div className="fixed inset-0 z-[180] grid place-items-center overflow-y-auto p-4">
          ...
          <motion.div
            ...
            className="relative my-8 w-full max-w-3xl rounded-[2rem] border border-white/10 bg-ink-900 shadow-2xl"
          >
            <button
              onClick={close}
              className="absolute left-4 top-4 z-10 grid size-10 place-items-center rounded-xl border border-white/10 bg-ink-950/60 text-frost-300 transition-colors hover:bg-white/10"
              aria-label="إغلاق"
            >
              <X className="size-5" />
            </button>
            ...
            {stage === "success" ? (
              <div className="grid place-items-center p-10 py-16 text-center">
                ...
                <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="rounded-2xl bg-gradient-to-l from-emerald-400 to-emerald-600 px-8 py-3.5 font-display font-black text-ink-950 transition-transform hover:scale-105"
                  >
                    إرسال الطلب على واتساب
                  </a>
                  <button
                    onClick={close}
                    className="rounded-2xl border border-white/10 px-8 py-3.5 font-bold text-frost-300 transition-colors hover:bg-white/5"
                  >
                    متابعة التسوق
                  </button>
                </div>
              ...
              <form onSubmit={submit} className="grid lg:grid-cols-[1.15fr_0.85fr]">
                        ...
                        {paymentMethods.map((m) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className={cn("mt-2 text-xs font-extrabold", active ? "text-white" : "text-frost-300")}>
                                {m.name}
                              </p>
                ...
                {/* summary side */}
                <div className="flex flex-col border-t border-white/[0.07] bg-ink-950/40 p-6 md:p-8 lg:border-r lg:border-t-0">
                  ...
                  <div className="mt-5 space-y-2 border-t border-white/[0.07] pt-4 text-sm">
                    ...
                    <div className="flex justify-between border-t border-white/[0.07] pt-3 font-display text-xl font-black">
                      <span>الإجمالي</span>
                      <span className="text-white">{fmt(total)} <span className="text-xs text-frost-400">جنيه</span></span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="mt-6 flex w-full items-center justify-center gap-2.5 rounded-2xl bg-gradient-to-l from-volt-400 to-volt-600 py-4 font-display text-lg font-black text-ink-950 shadow-[0_14px_36px_rgba(34,211,238,0.35)] transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-70"
                  >
                    {stage === "sending" ? (
                      <>
