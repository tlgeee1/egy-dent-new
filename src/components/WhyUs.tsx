...
const features = [
  ...
  },
  {
    ...
    desc: "24 ساعة داخل القاهرة والجيزة، و2-4 أيام لباقي محافظات مصر.",
  },
  {
    icon: Headset,
...
const stats = [
  ...
  { to: 600, suffix: "+", label: "عيادة ومعمل بيتعاملوا معانا" },
  { to: 24, suffix: "h", label: "أقصى مدة توصيل داخل القاهرة" },
  { to: 15, suffix: "+", label: "سنة خبرة في السوق المصري" },
];
          ...
          <div>
            {features.map((f, i) => (
              <Reveal key={f.title} delay={i * 0.07}>
                <div className="group flex items-start gap-5 border-b border-[var(--line-2)] py-8 transition-all duration-500 first:pt-0 hover:pr-3">
                  <span className="font-display text-sm font-black text-frost-500 transition-colors group-hover:text-volt-400" dir="ltr">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="grid size-14 shrink-0 place-items-center rounded-2xl border border-volt-500/25 bg-volt-500/[0.08] text-volt-300 transition-all duration-500 group-hover:bg-volt-500 group-hover:text-[var(--onaccent)] group-hover:shadow-[0_12px_30px_rgba(34,211,238,0.4)]">
