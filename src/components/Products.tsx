...
function ProductCard({ p, index }: { p: Product; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex cursor-pointer flex-col overflow-hidden rounded-[1.75rem] border border-white/[0.07] bg-ink-900 transition-colors duration-500 hover:border-volt-500/40 hover:shadow-[0_24px_70px_rgba(6,182,212,0.12)]"
    >
      {/* image */}
      <div className="relative h-60 overflow-hidden">
        ...
        {p.badge && (
          <span className="absolute right-4 top-4 rounded-full bg-gradient-to-l from-gold-400 to-gold-500 px-3 py-1.5 font-display text-xs font-black text-ink-950 shadow-lg">
            {p.badge}
          </span>
        )}
        {discount > 0 && !p.badge && (
          <span className="absolute right-4 top-4 rounded-full bg-volt-500 px-3 py-1.5 font-display text-xs font-black text-ink-950">
            -{discount}%
          </span>
        )}

        {/* quick view hint */}
        <span className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-full bg-ink-950/70 px-5 py-2.5 text-xs font-bold text-white opacity-0 backdrop-blur-md transition-all duration-300 group-hover:opacity-100">
          <Eye className="size-4 text-volt-300" />
          عرض التفاصيل
        </span>

        {/* rating */}
        <span className="absolute bottom-3 left-4 flex items-center gap-1.5 rounded-full border border-white/10 bg-ink-950/70 px-3 py-1 text-xs font-bold text-white backdrop-blur-md" dir="ltr">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-5">
          ...
          <button
            ...
            className={cn(
              ...
              added
                ? "bg-emerald-400 text-ink-950 shadow-[0_10px_28px_rgba(52,211,153,0.4)]"
                : "bg-gradient-to-br from-volt-400 to-volt-600 text-ink-950 shadow-[0_10px_28px_rgba(34,211,238,0.3)] hover:scale-105",
            )}
          >
            ...
            {[{ id: "all", name: "كل المنتجات" }, ...categories].map((c) => (
              <button
                ...
                className={cn(
                  ...
                  filter === c.id ? "text-ink-950" : "border border-white/10 text-frost-300 hover:border-volt-500/40 hover:text-volt-300",
                )}
              >
                {filter === c.id && (
                  <motion.span
