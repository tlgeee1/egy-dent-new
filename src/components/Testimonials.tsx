...
function Card({ t }: { t: (typeof testimonials)[number] }) {
  return (
    <figure className="relative w-[320px] shrink-0 rounded-[1.5rem] border border-[var(--line-2)] bg-ink-800/70 p-6 backdrop-blur-sm transition-colors hover:border-volt-500/30 md:w-[380px]">
      ...
      <figcaption className="mt-5 flex items-center gap-3 border-t border-[var(--line-2)] pt-4">
        <span className="grid size-11 place-items-center rounded-full bg-gradient-to-br from-volt-400 to-volt-700 font-display text-lg font-black text-[var(--onaccent)]">
          {t.initial}
        </span>
        <div>
